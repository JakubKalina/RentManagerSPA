...
import { getFlatReports, createLandlordReport, getReports } from "App/state/landlord/reports/reports.thunk";
import { CreateLandlordReportRequest } from "App/api/endpoints/report/requests/createlLandlordReportRequest";
import { renderReportsTableColumns } from "./components/ReportsTable";


const { LOADING, SUCCESS } = StatusType;


const LandlordPageGetFlatReportsContainer: React.FC<{}> = () => {

    const dispatch = useDispatch();
    
	type FinishFormType = (values: Store) => void;

	const reports = useSelector((state: RootState) => state.landlord.reports.reports);
	const reportsStatus = useSelector((state: RootState) => state.landlord.reports.status);
			...
			<Row className='overflow-hidden'>
				<Col span={24}>
					<Table
						...
						loading={reportsStatus.getFlatReports === LOADING}
						columns={renderReportsTableColumns(reports, dispatch)}
						dataSource={reports}
						rowKey='id'
					/>
				</Col>
